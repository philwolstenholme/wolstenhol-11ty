...
<template>
  <pw-section section-key="reading">
    ...
    <ul role="list" class="mt-8 grid gap-5 pb-2 md:grid-cols-2 xl:grid-cols-3">
      <li
      ...
      >
        <div
          class="reading-list-link group relative flex min-w-0 flex-1 items-baseline space-x-3 rounded border border-gray-300 bg-oc-yellow-2 p-4 font-serif shadow-hard focus-within:bg-oc-yellow-3 hover:bg-oc-yellow-3"
        >
          <img
            :src="item.favicon"
            :srcset="!item.favicon.startsWith('data:') ? `${item.favicon.replace(/16/gi, 32)} 2x` : null"
            class="isolate flex-shrink-0 rounded"
            alt=""
            width="16"
            height="16"
            loading="lazy"
          ...
          <div class="isolate min-w-0">
            <h3 class="font-bold leading-tight group-hocus:underline">
              <a :href="item.url" class="group-hover:underline">{{ item.title }}</a>
            </h3>
            <p class="mt-2 truncate text-xs no-underline" v-if="item.subTitle">{{ item.subTitle }}</p>
          </div>
        </div>
      </li>
      <li class="scroll-saver space-y-3 no-js:hidden md:hidden" x-data>
        ...
        <button
          type="button"
          class="m-auto block rounded border border-yellow-100 bg-black px-4 py-2 font-serif font-bold text-white shadow-hard"
      ...
      >
        <svg
          ...
          width="208"
          class="absolute -left-56 top-1/4 hidden 2xl:inline"
        >
          <path
        ...
        <div
          class="reading-list-link focys-within:bg-oc-yellow-3 group relative flex min-w-0 flex-1 items-baseline space-x-3 rounded border border-gray-300 bg-oc-yellow-2 p-4 font-serif shadow-hard hover:bg-oc-yellow-3"
        >
          <img :src="item.favicon" class="isolate flex-shrink-0 rounded" alt="" width="16" height="16" loading="lazy" />
          <div class="isolate min-w-0">
